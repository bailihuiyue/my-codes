...
export default class CaseSearch extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      region: [],
  ...
  changeSelect = (type, value) => {
    ...
    switch (type) {
      case "province":
        ...
        // delete province also delete related region
  ...
  render() {
    ...
    const rowCfg = { row: { gutter: 16 }, col: new Array(19).fill({ xxl: 6, xl: 8, lg: 12 }) };
    const { mainViolation, submissionEnty, discussion, violatorType, currentStep, punishment, status, province, lastStep, caseCategory, bonusDeduction } = getDictionValues();
    const content = [
      {
        id: "dateFrom",
